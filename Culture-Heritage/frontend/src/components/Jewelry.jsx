...
const Jewelry = ({ setCartItems }) => {
  ...
  useEffect(() => {
    const fetchVegetables = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/products/type/jewelry');
  ...
  return (
    <div className="home-container">
      ...
      <div className="welcome-message">
        <h2>Jwellery</h2>
        <p>Adorn Yourself with Tradition: Timeless Indian Jewelry for Every Occasion.</p>
      </div>
      ...
      <div className="product-grid">
        {filteredProducts.length > 0 ? (
          ...
          <p>No jewelry products available at the moment.</p>
        )}
      </div>
    </div>
  );
