...
const Spcies = ({ setCartItems }) => {
  ...
  useEffect(() => {
    const fetchVegetables = async () => {
      try {
        const response = await axios.get('http://localhost:5000/api/products/type/spices');
  ...
  return (
    <div className="home-container">
      ...
      <div className="welcome-message">
        <h2>Spcies</h2>
        <p>"Savor the Flavors of India: Authentic Spices and Culinary Delights."</p>
      </div>
      ...
      <div className="product-grid">
        {filteredProducts.length > 0 ? (
          ...
          <p>No spcies available at the moment.</p>
        )}
      </div>
    </div>
  );
